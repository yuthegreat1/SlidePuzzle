...
def process_file(filename, algorithm, param):
    ...
    for line in file:
        myDigitStrings = line[:-1]
        init_board = Board(myDigitStrings)
        init_state = State(init_board, None, 'init')
        searcher = create_searcher(algorithm, param)
        if searcher == None:
            return 
        ...
        try:
            soln = searcher.find_solution(init_state)
            if soln == None:
                print(myDigitStrings + ':','no solution')
            else:
                  print(myDigitStrings + ':', soln.num_moves, 'moves,', searcher.num_tested, 'states tested')
                  puzzles += 1
                  moves += soln.num_moves
                  tested_states += searcher.num_tested
        except KeyboardInterrupt:
              print(myDigitStrings + ':', 'search terminated')
    print()
